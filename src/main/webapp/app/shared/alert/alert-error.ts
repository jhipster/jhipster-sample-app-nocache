...
export class AlertError implements OnDestroy {
  ...
  constructor() {
    this.errorListener = this.eventManager.subscribe('jhipsterNoCacheSampleApplicationApp.error', (response: EventWithContent<unknown>) => {
    ...
    this.httpErrorListener = this.eventManager.subscribe(
      'jhipsterNoCacheSampleApplicationApp.httpError',
  ...
  private handleFieldsError(httpErrorResponse: HttpErrorResponse): void {
    ...
    for (const fieldError of fieldErrors) {
      ...
      const fieldName: string = this.translateService.instant(
        `jhipsterNoCacheSampleApplicationApp.${fieldError.objectName as string}.${convertedField}`,
      );
